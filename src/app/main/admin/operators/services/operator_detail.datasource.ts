...
export class OperatorDetailDataSource extends DataSource<any>
{
    ...
    loadOperatorDetail(conncode: string, userid: number, pageindex: number, pagesize: number, name: string, method: string) {
        console.log("loadOperators:", conncode, userid,  pagesize, pageindex, name, method );
        if (!name) {
            name = '';
        }
        ...
        this.operatorDetailService.getCompanies(conncode, userid, pageindex, pagesize, name, method)
        .pipe(
           catchError(() => of([])),
        ...
        .subscribe((result : any) => {
           console.log("method:", method, result);
            ...
            console.log("unit_clist: ", this.operatorDetailService.unit_clist_item);
