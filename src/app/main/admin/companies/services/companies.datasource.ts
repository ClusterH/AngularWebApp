...
export class CompaniesDataSource extends DataSource<any> {
    ...
    constructor(private _adminCompaniesService: CompaniesService) {
        super(); this._unsubscribeAll = new Subject();
    }

    loadCompanies(pageindex: number, pagesize: number, orderby: string, orderdirection: string, filterItem: string, filterString: string, method: string) {
        this.loadingSubject.next(true);
        this._adminCompaniesService.getCompanies(pageindex, pagesize, orderby, orderdirection, filterItem, filterString, method)
            .pipe(
                catchError(() => of([])),
