...
import { SimcardsComponent } from "app/main/admin/simcards/simcards/simcards.component";
...
export class SimcardsDataSource extends DataSource<any>
{
    ...
    loadSimcards(conncode: string, userid: number, pageindex: number, pagesize: number, orderby: string, orderdirection: string, filterItem: string, filterString: string,method: string) {
        console.log("loadSimcards:", conncode, userid,  pagesize, pageindex, orderdirection, orderby, filterItem, filterString, method );
        this.loadingSubject.next(true);
   
        // use pipe operator to chain functions with Observable type
        this._adminSimcardsService.getSimcards(conncode, userid, pageindex, pagesize, orderby,  orderdirection, filterItem, filterString, method)
        .pipe(
           catchError(() => of([])),
