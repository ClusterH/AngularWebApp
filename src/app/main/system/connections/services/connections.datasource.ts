...
export class ConnectionsDataSource extends DataSource<any> {
    ...
    constructor(private _systemConnectionsService: ConnectionsService) {
        super();
        this._unsubscribeAll = new Subject();
    }

    loadConnections(pageindex: number, pagesize: number, orderby: string, orderdirection: string, filterItem: string, filterString: string, method: string) {
        this.loadingSubject.next(true);
        this._systemConnectionsService.getConnections(pageindex, pagesize, orderby, orderdirection, filterItem, filterString, method)
            .pipe(
                catchError(() => of([])),
