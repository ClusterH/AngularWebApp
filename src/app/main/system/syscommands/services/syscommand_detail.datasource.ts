...
export class SysCommandDetailDataSource extends DataSource<any> {
    ...
    constructor(private syscommandDetailService: SysCommandDetailService) {
        super();
        this.flag = false; this._unsubscribeAll = new Subject();
    }

    loadSysCommandDetail(pageindex: number, pagesize: number, name: string, method: string) {
        if (!name) { name = ''; }
        this.loadingSubject.next(true);
        this.syscommandDetailService.getCompanies(pageindex, pagesize, name, method)
            .pipe(
                catchError(() => of([])),
