...
export class UnittypesDataSource extends DataSource<any> {
    ...
    constructor(private _systemUnittypesService: UnittypesService) {
        super();
        this._unsubscribeAll = new Subject();
    }

    loadUnittypes(pageindex: number, pagesize: number, orderby: string, orderdirection: string, filterItem: string, filterString: string, method: string) {
        this.loadingSubject.next(true);
        this._systemUnittypesService.getUnittypes(pageindex, pagesize, orderby, orderdirection, filterItem, filterString, method)
            .pipe(
                catchError(() => of([])),
