...
export class MonitorDataSource extends DataSource<any> {
    ...
    loadClists(pageindex: number, pagesize: number, name: string, method: string) {
        if (!name) { name = ''; }
        this.loadingSubject.next(true);
        this.monitorService.getClists(pageindex, pagesize, name, method)
            .pipe(
                catchError(() => of([])),
            ...
            .subscribe((result: any) => {
                ...
                this.monitorService.unit_clist_item[`${method}`] = result.TrackingXLAPI.DATA || [];
